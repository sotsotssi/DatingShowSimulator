...
const LOCATIONS = [
    "거실", "주방", "테라스", "수영장",
    "공원", "산", "놀이공원", "레스토랑", "카페", "바다", "광장",
    "도서관", "영화관", "노래방", "헬스장", "루프탑 바", "식물원", "미술관", "쇼핑몰", "캠핑장",
    "불꽃놀이 축제"
];
...
function getActionType(location) {
    switch (location) {
        ...
        case "레스토랑": case "영화관": case "루프탑 바": return "date";
        case "놀이공원": return "ride"; 
        case "수영장": case "노래방": return "leisure";
        case "헬스장": case "산": return "exercise";
        case "미술관": return "art";
        case "쇼핑몰": return "shopping";
        case "공원": case "바다": case "광장": case "테라스": case "식물원": return "walk";
...
function generateActionScript(actionType, isSuccess, actor, target, loc) {
    ...
    const acts = {
        ...
        leisure: {
            success: [
                `${loc}에서 ${josa(actor.name, '은/는')} 숨겨둔 개인기를 뽐내 ${target.name}의 감탄을 자아냈습니다.`,
        ...
        exercise: {
            success: [
                ...
                `${josa(actor.name, '은/는')} ${target.name}에게 운동 자세를 친절하게 가르쳐주었습니다.`,
                `${josa(actor.name, '은/는')} 지치지 않는 체력을 과시하며 ${target.name}에게 든든한 모습을 보여주었습니다.`
            ],
            fail: [
                `${josa(actor.name, '은/는')} 무리하게 운동하다 쥐가 나 ${target.name}의 부축을 받아야 했습니다.`,
                `${josa(actor.name, '은/는')} 저질 체력을 드러내며 ${target.name}보다 먼저 지쳐버렸습니다.`,
                `${actor.name}의 땀 냄새가 신경 쓰였는지 ${josa(target.name, '이/가')} 살짝 거리를 두었습니다.`
            ]
        },
        cooking: {
            success: [
                ...
                `${josa(actor.name, '은/는')} ${josa(target.name, '과/와')} 함께 ${josa(dish, '을/를')} 만들며 알콩달콩한 분위기를 연출했습니다. 맛도 훌륭합니다.`
            ],
            fail: [
        ...
        ride: { 
            ...
            fail: [
                ...
                `${actor.name}의 ${ride} 안전 장치 문제로 잠시 소동을 피웠습니다. 부끄러운 순간입니다.`
            ]
        },
        movie: {
            success: [
            ...
            fail: [
                `${actor.name}이(가) 고른 ${josa(movie, '은/는')} 너무 지루해서 ${josa(target.name, '이/가')} 졸기 시작했습니다.`,
        ...
        shopping: {
            success: [
                `${josa(actor.name, '은/는')} ${target.name}에게 딱 어울리는 옷을 골라주며 패션 센스를 자랑했습니다.`,
                `${josa(actor.name, '은/는')} ${josa(target.name, '과/와')} 서로의 물건을 골라주며 커플 아이템을 맞추는 재미를 느꼈습니다.`,
                `${josa(actor.name, '은/는')} 짐을 모두 들어주며 ${josa(target.name, '을/를')} 배려하는 모습을 보여주었습니다.`
            ],
            fail: [
    ...
    if (actionType === 'leisure') {
        if (loc === '노래방') return generateActionScript('singing', isSuccess, actor, target, loc);
    }
    ...
    const icon = isSuccess ? 
        ["✨", "🍳", "🍷", "🎾", "💬", "🎡", "🎤", "💪", "🎨", "🛍️", "🎬"][["walk","cooking","date","leisure","talk","ride","singing","exercise","art","shopping","movie"].indexOf(typeKey)] || "✨" 
        : "💦";
    
    let displayType = '대화';
    if(typeKey === 'cooking') displayType = '요리';
    else if(typeKey === 'date') displayType = '데이트';
    else if(typeKey === 'leisure') displayType = '여가';
    else if(typeKey === 'singing') displayType = '노래';
    else if(typeKey === 'exercise') displayType = '운동';
    else if(typeKey === 'art') displayType = '관람';
    else if(typeKey === 'shopping') displayType = '쇼핑';
    else if(typeKey === 'movie') displayType = '영화';
    else if(typeKey === 'walk') displayType = '산책';
    else if(typeKey === 'ride') displayType = '놀이';

    return `${icon} [${displayType}/${isSuccess ? '성공' : '실패'}] ${msg}`;
}
...
function processNextDay() {
    ...
    if (activeChars.length < 2) return alert('최소 2명의 활성 참가자가 필요합니다.');
    if (state.ended) return alert('시뮬레이션이 종료되었습니다. 다시 시작하려면 버튼을 누르세요.');

    state.day++;
    document.getElementById('currentDay').innerText = state.day;
    document.getElementById('progressText').innerText = `${state.day}일차 진행 중`;
    ...
    for (let i = 0; i < shuffledChars.length; i++) {
        ...
        for (const candidate of candidates) {
            ...
            if (rel.affection < 0) {
                if (Math.random() < 0.8) {
                    dailyLogs.push(`💔 [회피] ${josa(actor.name, '은/는')} ${josa(candidate.name, '과/와')} 마주칠 뻔했으나, 불편한 마음에 자리를 피했습니다.`);
                    continue; 
                }
            }
            target = candidate;
...
function triggerSoloEvent(actor, logs) {
    ...
    logs.push(`🧘 [단독] ${josa(actor.name, '은/는')} ${loc}에서 ${msg}`);
}
...
function triggerEvent(actor, target) {
    ...
    if (actor.coupleId && actor.coupleId !== target.id) {
        ...
        if (state.config.allowAffair && Math.random() * 100 < cheatProb) {
             ...
             return `💔 [외도] ${josa(actor.name, '이/가')} 연인 ${partner.name} 몰래 ${josa(target.name, '과/와')} ${loc}에서 밀회를 즐겼습니다.`;
        } else {
             return `🛡️ [철벽] ${josa(actor.name, '은/는')} ${josa(target.name, '과/와')}의 시간에서 연인 ${partner.name} 생각만 했습니다.`;
        }
    }
    ...
    if (isFireworks) {
        return `🎆 [축제] ${josa(actor.name, '과/와')} ${josa(target.name, '은/는')} 불꽃놀이를 보며 로맨틱한 분위기에 취했습니다.`;
    }
